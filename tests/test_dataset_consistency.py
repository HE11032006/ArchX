...
no foreign-stack mentions, no undisclosed stack/age/team-composition mentions
...
to_review.jsonl by dataset_generation/reconcile_flagged_examples.py.

Also guards against a second, distinct hallucination bug found via a manual
Gemma 4 inference test: teacher_prompts.py used to hand the teacher the
project's age and junior/senior split and explicitly ask it to "naturally
incorporate" that context into project_description — so ~92% (458/500) of
examples narrated facts (age, team composition) that are never in the
student's own input. The prompt was fixed to mark that context
internal-reasoning-only, and Scenario.stack_known now lets ~25% of examples
train the "stack not detected -> don't mention one" case too.
"""

import json
from pathlib import Path

from dataset_generation.generate_dataset import (
    _foreign_stack_mentions,
    _recommends_migration,
    _undisclosed_context_mentions,
    _undisclosed_stack_mentions,
)
...
def test_no_healthy_or_critical_inconsistency_in_training_data():
    ...
    for record in records:
        ...
        description = output.get("project_description", "")
        if scenario:
            if scenario.stack_known and _foreign_stack_mentions(description, scenario):
                violations.append((record["scenario_id"], "foreign-stack"))
            if not scenario.stack_known and _undisclosed_stack_mentions(description):
                violations.append((record["scenario_id"], "undisclosed-stack"))

        if _undisclosed_context_mentions(output):
            violations.append((record["scenario_id"], "undisclosed-age-or-team"))
